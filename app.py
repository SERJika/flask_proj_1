...
def main():
    ...
    output = render_template("main.html", **template_context)  # return 'Здесь будет главная'
    return output

@app.route('/from/<direction>')
def direction(direction):
    if direction not in departures.keys():
        abort(404)
    direction_tours = []
    i = 1
    for key, value in tours.items():
        if value['departure'] == direction:
            tours[key]['id'] = i
            direction_tours.append(tours[key])
        i += 1;

    template_context = dict(
        homepage_title=homepage_title,
        title='Туры '+departures[direction],
        subtitle=subtitle,
        description=description,
        departures=departures,
...
def tour(id):
    ...
    output = render_template("tour.html", **template_context)  # return 'Здесь будет тур'
    return output
...
def page_not_found(error):
    ...
    output = render_template("404.html", **template_context)  # return 'Здесь будет тур'
    return output
